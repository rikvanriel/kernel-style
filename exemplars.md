...
Canonical routing table: [`exemplars-routing.md`](./exemplars-routing.md) (180w tiny, hot-loadable) — single source of truth per CONTRIBUTING §2 one-source-of-truth. This intro previously duplicated that table; now it cross-references to avoid duplication.

When you are at Phase 2 review, load `exemplars-routing.md` first (not this full file) to pick one developer profile based on bug class. Then load only that section from this file on demand via `scripts/phases.py --phase 2 --bug-class <class> --extract-only` or `scripts/phases.py --phase 2 --extract "<Developer>" --extract-only`. This saves ~3,300w / ~5.5k tok vs loading full 3,685w file. The routing file is canonical; if you need full context, `scripts/phases.py --list-profiles` shows all 14 profiles with word counts.

If none of the bug classes fit, pick subsystem-closest developer per per-developer subsystem lists at each profile header. If in doubt, skim one-line signature strengths in `kernel-readability-principles.md` § Signature strength.

This file contains the full per-developer profiles below — reference detail behind `kernel-readability-principles.md`. Do not rely on anchors alone for voice calibration; compare diff against relevant section.
