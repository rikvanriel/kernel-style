...
This file is the slim entry point (<1000 words). Detailed changelog and code-comment rules — subject lines, body structure, verbatim artifacts, paragraph caps, audience relevancy, trailers, comment density, LLM-slop contrasts — live in **[changelog-style.md](./changelog-style.md)**. Load that file when you are actually drafting or reviewing a patch message or adding comments. Related files: [kernel-readability-principles](./kernel-readability-principles.md) for why, [llm-tells-checklist](./llm-tells-checklist.md) for generic tells to strip, [exemplars](./exemplars.md) for annotated real commits, [patch-series](./patch-series.md) for multi-patch structure. Each rule ID also has a matching entry in the sibling `*-rationale.md`, which is not loaded by default: read it when you are unsure how to apply a rule, or before disputing one. See README for load order.
...
- <!-- R0-8 --> **Name the artifact behind every empirical claim before you commit, and for a measurement name the control too.** For each number, symptom, splat, or test result in the changelog, be able to point at what produced it (log path, command output, hash). Delete or mark TODO anything you cannot. A benchmark needs one more thing: whatever proves it measured the case you claim — the counter showing the intended folio size was actually allocated, the `uname -r` showing the intended kernel booted. Without it a mis-set knob yields plausible numbers that nobody can challenge.
