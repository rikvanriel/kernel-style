...
- `peer-review.md` — review process specification with three mandatory questions "what's wrong?", "is there a materially better way?", and "can the same correctness be achieved with less structural duplication?", run as self-review by default and escalated to a second reviewer when one is available. Load mandatory during Phase 2 review and Phase 3 changelog drafting.
...
3. Load peer-review.md mandatory during Phase 2 review. Run the three mandatory questions per peer-review.md as a self-review of your own diff and commit message draft first — this is the default, always-available gate, not optional even when working alone:
 ...
 * (c) Can the same correctness be achieved with less structural duplication? — scan the diff for mechanical simplification: same guard, lock, or RCU read duplicated per object or variant → hoist to the loop or function top; same test-set-queue pattern duplicated per variant → extract a helper or handle the fast path together; nested conditionals → flatten to early continue or return. Sketch a concrete 3–6 line before and after or state why already minimal. Do not dismiss as minimal obvious fix when the change itself introduced the duplication.
