...
4. Re-run llm-tells-checklist.md final pass against diff output to verify no LLM tells slipped in during drafting. Checklist is always hot, so no extra load needed beyond re-running with current diff context. Optional machine helper: `scripts/lint-changelog.py` for paragraph caps [CL-12], banned phrases [CL-13], Fixes+Cc pairing [CL-11], internal IDs [CL-14].
