...
- <!-- CL-35 --> Compression axis. One changelog for a 158/255-line rbtree-to-maple-tree conversion in drivers/iommu/iova.c: 401 words in 11 paragraphs (max 54). An LLM asked to shorten it deleted three paragraphs and reached 320 words in 8 (max 47). The author instead rewrote every paragraph in place and reached 282 words in 10 (max 45) — fewer words in more paragraphs. The deletions had removed claims (CL-35) while leaving the surviving prose uncompressed.

- <!-- CL-36 --> Scope boundary versus comparative reassurance. Same changelog. The author instructed "take out commentary about things that stay the same, and about things that are well known (eg. maple tree tracking gaps)", then kept both "The rcache layer is unchanged. It deals in raw pfn values and does not care what the ranges are kept in" and "The tree tracks gaps itself, so the IOVA_ANCHOR sentinel and the cached_node and cached32_node fields are gone", while cutting "free_iova(), find_iova() and reserve_iova() were O(log n) with the rbtree already, and stay that way" and "The VMA code uses the same structure for the same job". The kept lines answer "what do I not need to review" and "why is this field gone"; the cut lines answer neither. A narrower reading was considered and rejected: that the instruction's own "well known" category explains every choice, since a complexity class and a code-reuse analogy are both common knowledge. It does not explain the two that survived, as neither is well known — both are facts about this diff, and each answers a question a reviewer would otherwise have to ask. Scope boundary, not reassurance, is what separates them.

- <!-- CL-37 --> Mechanism versus tradeoff. Same changelog, alignment paragraph. Draft: "Alignment is handled by requesting size + alignment - 1 pfns, which leaves room to round up inside the range that comes back, so the retry loop goes away." Final: "Alignment is handled by rounding up the allocation size, prioritizing speed over address space waste, with the thought that many iova requests on a system will be similar in size, and reuse the same holes." Same length, and the second discloses an accepted cost plus the workload assumption behind it. "So the retry loop goes away" went too, being visible in the diff.

- <!-- CL-38 --> Removals and derived numbers. Same changelog. Draft: "the slab cache no longer needs SLAB_HWCACHE_ALIGN, which had rounded each iova up to 64. The maple nodes that replace those pointers cost roughly 28-41 bytes per entry at typical fill." Final: "Remove SLAB_HWCACHE_ALIGN to pack 4 iova structs in a cache line." The 28-41 range was computed from maple node fill assumptions, not measured, so it was deleted rather than defended; 4 per 64-byte line follows from the 16-byte struct the same paragraph states.

- <!-- CL-39 --> Ordering multiple reasons. Same changelog, lock paragraph. MT_FLAGS_LOCK_EXTERN has two justifications: the iova code runs in interrupt context and needs an irq-safe lock, which mtree_lock()'s plain spin_lock() is not; and one critical section spans a gap search plus the store that fills it. The draft led with the second and trailed the first as a clause; the final led with the interrupt-context requirement and compressed the rest to "The same lock also protects related iova data". Only the first makes the tree's built-in lock unusable.
