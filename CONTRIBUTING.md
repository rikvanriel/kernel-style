...
- Hot files contain normative checkable rules only. Max soft budget per file: kernel-style.md <1,000 words, kernel-readability-principles.md <1,000, llm-tells-checklist.md <800, changelog-style.md <6,000 (Phase 3 only, re-baselined 2026-08-17, see §6), patch-series.md <1,200 after slim, exemplars.md unchanged but moved to on-demand. Total hot-set token budget is tracked, not per-file hard cap — see measurement section below.
...
- Soft targets (reassess quarterly):
  ...
  * Phase 3 multi-patch with patch-series ≤21,000 tokens
  ...
  Phase 3 target re-baselined 2026-08-17 (owner decision): changelog-style.md gets <6,000 words rather than <2,000. It loads only once a patch is drafted and reviewed, on a turn already dominated by iterating on the code and checking that the changelog and the diff agree, so a few thousand reference tokens are a small share of the total. This covers the phase-triggered file only. The four Tier 1 budgets stay as they are, since those are resident on every turn and pay their cost whether or not a changelog is being written. A review finding of the form "changelog-style.md is over its budget" is answered by this paragraph; a finding that a specific rule duplicates an existing one still stands on its own merits.
